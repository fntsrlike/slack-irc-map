...
Command.prototype._create = function(args) {
  if (args.length !== 1) {
    throw new CommandError('Invalid formation! There just a argument with create action.');
  }
  var ircName = args[0];
...
Command.prototype._update = function(args) {
  if (args.length !== 2) {
    throw new CommandError('Invalid formation! There need 2 arguments with update action.');
  }
...
Command.prototype._delete = function(args) {
  if (args.length !== 1) {
    throw new CommandError('Invalid formation! There just a argument with delete action.');
  }
  var ircName = args[0];
...
Command.prototype.execute = function(text) {
  ...
  if (this.validSubCommand.indexOf(this.subCommand) === -1) {
    throw new CommandError('Invalid sub command! Please use help action to check valid options');
  }
